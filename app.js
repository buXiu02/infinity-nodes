...
function normaliseNode(n){if(!Array.isArray(n.tags))n.tags=[];n.tags=[...new Set(n.tags.map(t=>String(t).trim().replace(/^#+/,'')).filter(Boolean))];if(!['standard','main','parent','title'].includes(n.type))n.type='standard';if(n.portalProjectId!==null&&typeof n.portalProjectId!=='string')n.portalProjectId=null;if(!Number.isFinite(n.opacity))n.opacity=1;if(n.imageData){if(!Number.isFinite(n.imageWidth))n.imageWidth=240;if(!Number.isFinite(n.imageHeight))n.imageHeight=180;}if(!['none','canvas','frame'].includes(n.pinMode))n.pinMode='none';if(n.pinMode!=='frame')n.pinFrameId=null;normaliseNote(n);return n}
...
function copyFrame(){let f=frameById(selected?.id);if(!f)return;let b=frameBounds(f),inside=nodesInsideFrame(f),ids=new Set(inside.map(n=>n.id)),map=new Map(),copies=inside.map(n=>{let x=structuredClone(n);let old=x.id;x.id=uid();map.set(old,x.id);x.x-=b.x;x.y-=b.y;if(x.pinMode==='frame'&&x.pinFrameId===f.id)x.pinFrameId='__COPIED_FRAME__';return x}),edges=state.edges.filter(e=>ids.has(e.a)&&ids.has(e.b)).map(e=>{let x=structuredClone(e);x.id=uid();x.a=map.get(e.a);x.b=map.get(e.b);return x}),fc=structuredClone(f);fc.id=uid();for(const x of copies)if(x.pinFrameId==='__COPIED_FRAME__')x.pinFrameId=fc.id;fc.points=fc.points.map(p=>({x:p.x-b.x,y:p.y-b.y}));let target;if(frameTarget.value==='__new__'){target=freshProject(f.title||'Frame');db.projects.push(target)}else target=db.projects.find(p=>p.id===frameTarget.value);if(!target)return;target.frames=target.frames||[];target.frames.push(fc);target.nodes.push(...copies);target.edges.push(...edges);target.updated=Date.now();save();renderProjects();alert(`Copied “${f.title}” with ${copies.length} node${copies.length===1?'':'s'} to ${target.name}.`)}
...
function syncNodePortalPanel(n){if(!nodePortalProject)return;portalTargets(nodePortalProject,n.portalProjectId);openNodePortal.hidden=!nodePortalProject.value}
...
nodePortalProject.onchange=()=>{if(selected?.type!=='node')return;let n=nodeById(selected.id);if(!n)return;checkpoint();n.portalProjectId=nodePortalProject.value||null;openNodePortal.hidden=!n.portalProjectId;save();render();colorPanel.hidden=false;syncNodePortalPanel(n)};
openNodePortal.onclick=()=>{let n=selected?.type==='node'&&nodeById(selected.id);if(n?.portalProjectId)openPortal(n.portalProjectId)};
...
// V20 — Templates. Kept isolated from the core graph system.
{
  ...
  function stripPortal(x){if(x&&'portalProjectId' in x)x.portalProjectId=null;return x}
